...
# Keep a reference to the real implementation before conftest.py's
# autouse fake_llm fixture replaces llm._ask_llm during each test.
ORIGINAL_ASK_LLM = llm._ask_llm
...
def test_ask_llm_configures_timeout_and_retries(monkeypatch):
    captured = {}

    async def fake_create(*args, **kwargs):
        message = type("Message", (), {"content": "AI response"})()
    ...
    result = asyncio.run(ORIGINAL_ASK_LLM("system", "user"))
...
def test_ask_llm_converts_sdk_timeout_to_service_timeout(monkeypatch):
    ...
    monkeypatch.setattr(llm, "APITimeoutError", FakeAPITimeoutError)
    monkeypatch.setattr(llm, "AsyncOpenAI", FakeAsyncOpenAI)
    monkeypatch.setattr(llm, "api_key", "test-key")
    monkeypatch.setattr(llm, "client", None)

    with pytest.raises(llm.AIServiceTimeoutError, match="30 seconds"):
        asyncio.run(ORIGINAL_ASK_LLM("system", "user"))